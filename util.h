...
{	/*
	Nico:
	代码片段 Beep(300, 200); Beep(200, 150); 用于在 Windows 操作系统中生成简单的音频提示音。
	Beep 是一个 Windows API 函数，它位于 kernel32.dll 动态链接库中。该函数用于通过计算机的扬声器或蜂鸣器发出声音。
	在这个特定的代码片段中，Beep 函数被调用两次，每次传递不同的参数。
	第一次调用 Beep(300, 200) 会发出一个短暂的音频信号。参数 300 指定了音调的频率（单位为赫兹），这里是 300Hz。
	参数 200 指定了音频的持续时间（单位为毫秒），这里是 200 毫秒。因此，这个调用会发出一个频率为 300Hz 的声音，持续时间为 200 毫秒。
	第二次调用 Beep(200, 150) 也会发出一个短暂的音频信号。
	参数 200 指定了音调的频率（单位为赫兹），这里是 200Hz。
	参数 150 指定了音频的持续时间（单位为毫秒），这里是 150 毫秒。因此，这个调用会发出一个频率为 200Hz 的声音，持续时间为 150 毫秒。
	这些代码片段可以用来在程序中表示某种失败或错误的提示音效。
	*/
...
static DECLSPEC_NORETURN void DoubleBeepExit()
{
	...
	/* Nico:
	ExitProcess(0xc0000135) 是一个函数调用，它在 Windows 操作系统中用于终止当前正在运行的进程。
	参数 0xc0000135 是一个错误代码。
	在这种情况下，0xc0000135 表示一个特定的错误，即 STATUS_DLL_NOT_FOUND。
	当一个应用程序或进程依赖于某个动态链接库（DLL）文件，但系统无法找到该文件时，就会引发这个错误。
	*/
